...
async function sendMessageAtRandomInterval(token, callback) {
  ...
  setTimeout(() => {
    const signatureTypeList = [
      "simple",
      "advance(biometrics)",
      "advance(digital certificate)",
      "qualified",
    ];
    const signatureType =
      signatureTypeList[faker.number.int({ min: 0, max: 3 })];
...
const generateTinybirdPayload = async (
  token,
  signatureID,
  signatureType,
  since,
  ...
  const accountId1 =
    account_id_list[
      faker.number.int({ min: 0, max: account_id_list.length - 1 })
    ];
  created_on = faker.date.soon({ days: 1, refDate: created_on });
  // status either in_queue or ready
  let subscriptionPayload = generateSignaturePayload(
    accountId1,
    statusList[faker.number.int({ min: 0, max: 1 })],
    signatureType,
    signatureID,
    since,
    until,
  ...
  created_on = faker.date.soon({ days: 1, refDate: created_on });
  subscriptionPayload = generateSignaturePayload(
    accountId2,
    "signing",
    signatureType,
    signatureID,
...
const main = async () => {
  try {
    ...
    await sendMessageAtRandomInterval(
      token,
      async (token, signatureID, signatureType, since, until, created_on) => {
        generateTinybirdPayload(
          token,
          signatureID,
          signatureType,
          since,
